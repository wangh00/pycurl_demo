...
do_curl_impersonate(CurlObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "si", &ua, &num))
        return NULL;
    if (check_curl_state(self, 1 | 2, "setopt") != 0)
        return NULL;

    res = curl_easy_setopt(self->handle, CURLOPT_USERAGENT, NULL);
    if (res != CURLE_OK) {
        return NULL;
    }
    ...
    assert(ua != NULL);
    res = curl_easy_impersonate(self->handle, ua, num);
    /* Check for errors */
    if (res != CURLE_OK) {
        /* Failed to match any of the function signatures -- return error */
        PyErr_SetString(PyExc_TypeError, "invalid arguments to curl_impersonate");
        return NULL;
    }
...
PYCURL_INTERNAL PyMethodDef curlobject_methods[] = {
    ...
    {"impersonate", (PyCFunction)do_curl_impersonate, METH_VARARGS, curl_setopt_string_doc},
